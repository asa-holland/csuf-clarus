...
def export_pdf():
    ...
    contradictions = analysis.get("contradictions", [])
    s2_findings = analysis.get("s2_findings", [])
    ...
    _CONTRA_CODE = {"semantic": "FAIL-003", "modality": "FAIL-004"}

    if contradictions:
        cont_rows = [["#", "Code", "Type", "Conf.", "Statement A", "Statement B"]]
        for i, c in enumerate(contradictions, 1):
    ...
    if s2_findings:
        ...
        story.append(Spacer(1, 0.1 * inch))
        story.append(Paragraph("All Findings", sub_h))
